...
function DealFormModal({ deal, stages, clients, onSave, onClose, saving }: DealFormProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="bg-[var(--color-bg)] rounded-xl shadow-2xl w-full max-w-lg border border-[var(--color-border)]">
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          ...
          <div>
            ...
            <input
              ...
              className="w-full px-3 py-2 rounded-lg border border-[var(--color-border)] bg-[var(--color-bg)] text-[var(--color-text-primary)] text-sm focus:outline-none focus:ring-2 focus:ring-[var(--color-accent)]"
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              <label className="block text-xs font-medium text-[var(--color-text-muted)] mb-1">Client</label>
              <select
                value={clientId}
            ...
            <div>
              <label className="block text-xs font-medium text-[var(--color-text-muted)] mb-1">Stage</label>
              <select
                value={stageId}
          ...
          <div>
            ...
            <textarea
              ...
              rows={2}
              className="w-full px-3 py-2 rounded-lg border border-[var(--color-border)] bg-[var(--color-bg)] text-[var(--color-text-primary)] text-sm resize-none focus:outline-none focus:ring-2 focus:ring-[var(--color-accent)]"
          ...
          <div className="flex gap-3 pt-1">
            <button type="button" onClick={onClose}
              className="flex-1 py-2 rounded-lg border border-[var(--color-border)] text-sm text-[var(--color-text-secondary)] hover:bg-[var(--color-bg-hover)]">
              Cancel
            </button>
...
function DealCard({ deal, stages, onEdit, onDelete, onMove }: DealCardProps) {
  ...
  return (
    <div className={cn(
      "bg-[var(--color-bg)] rounded-lg border p-3 space-y-2 shadow-sm group hover:shadow-md transition-shadow",
      ...
      <div className="flex items-start justify-between gap-2">
        <Link href={`/deals/${deal.id}`} className="text-sm font-medium text-[var(--color-text-primary)] hover:text-[var(--color-accent)] line-clamp-2 leading-snug">
          {deal.title}
        </Link>
        <div className="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
          <button onClick={() => onEdit(deal)} className="p-1 rounded hover:bg-[var(--color-bg-hover)] text-[var(--color-text-muted)]">
            <Pencil className="w-3 h-3" />
          </button>
          <button onClick={() => onDelete(deal)} className="p-1 rounded hover:bg-red-50 text-[var(--color-text-muted)] hover:text-red-600">
      ...
      <div className="flex items-center justify-between">
        ...
        <span className="text-xs text-[var(--color-text-muted)]">{deal.probability}%</span>
      </div>

      {/* Close date */}
      {deal.expected_close && (
        <div className={cn("flex items-center gap-1 text-xs", overdue ? "text-red-600" : "text-[var(--color-text-muted)]")}>
      ...
      {deal.status === "open" && (
        <div className="relative">
          <button
            ...
            className="w-full flex items-center justify-center gap-1 py-1 text-xs text-[var(--color-text-muted)] hover:text-[var(--color-accent)] hover:bg-[var(--color-bg-hover)] rounded transition-colors"
          >
          ...
          {moveOpen && (
            <div className="absolute bottom-full left-0 right-0 mb-1 bg-[var(--color-bg)] border border-[var(--color-border)] rounded-lg shadow-lg z-10 overflow-hidden">
              {stages.filter(s => s.id !== deal.stage_id).map(s => (
                <button
                  key={s.id}
                  onClick={() => { onMove(deal.id, s.id); setMoveOpen(false); }}
                  className="w-full flex items-center gap-2 px-3 py-2 text-xs text-left hover:bg-[var(--color-bg-hover)] text-[var(--color-text-primary)]"
                >
...
export default function DealsPage() {
  ...
  useEffect(() => {
    Promise.all([
      fetch("/api/deal-stages").then(r => r.json()),
      fetch("/api/clients?limit=200").then(r => r.json()),
    ]).then(([stagesRes, clientsRes]) => {
  ...
  // AI: detect buying intent keywords in recent messages (demo: check localStorage convs)
  useEffect(() => {
    const BUY_KEYWORDS = ["buy","purchase","pricing","quote","proposal","contract","budget","subscribe","upgrade","license","demo","invest"];
    const raw = localStorage.getItem("ventra_conversations");
    if (!raw || aiTitle) return;
    try {
      const convs = JSON.parse(raw) as Array<{ title?: string; messages?: Array<{ content?: string }> }>;
      for (const conv of convs) {
        const msgs = conv.messages ?? [];
        for (const msg of msgs) {
          const text = (msg.content ?? "").toLowerCase();
          if (BUY_KEYWORDS.some(k => text.includes(k))) {
            setAiTitle(conv.title ? `Deal — ${conv.title}` : "New Deal from conversation");
            return;
          }
        }
      }
  ...
  async function handleDelete() {
    if (!delTarget) return;
    const res = await fetch(`/api/deals/${delTarget.id}`, { method: "DELETE" });
    if (res.ok) {
      ...
      <div className="flex-1 overflow-auto p-6">
        ...
        <div className="flex gap-3 mb-5 flex-wrap">
          {[
            ...
            <div key={p.label} className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-[var(--color-bg-secondary)] border border-[var(--color-border)]">
        ...
        <div className="flex items-center gap-3 mb-5 flex-wrap">
          ...
          <div className="relative flex-1 min-w-[200px]">
            ...
            <input
              ...
              className="w-full pl-9 pr-3 py-2 rounded-lg border border-[var(--color-border)] bg-[var(--color-bg)] text-sm text-[var(--color-text-primary)] placeholder:text-[var(--color-text-muted)] focus:outline-none focus:ring-2 focus:ring-[var(--color-accent)]"
            />
          </div>
          ...
          <select
            ...
            className="px-3 py-2 rounded-lg border border-[var(--color-border)] bg-[var(--color-bg)] text-sm text-[var(--color-text-primary)] focus:outline-none focus:ring-2 focus:ring-[var(--color-accent)]"
          >
              ...
              <button key={v} onClick={() => setView(v)}
                className={cn(
                  ...
                  view === v
                    ...
                    : "bg-[var(--color-bg)] text-[var(--color-text-muted)] hover:bg-[var(--color-bg-hover)]",
                )}>
        ...
        {!loading && deals.length === 0 && (
          <div className="flex flex-col items-center justify-center py-20 text-center">
            <TrendingUp className="w-10 h-10 text-[var(--color-text-muted)] mb-3" />
            <p className="font-medium text-[var(--color-text-primary)]">No deals yet</p>
            <p className="text-sm text-[var(--color-text-muted)] mt-1 mb-4">Create your first deal to start tracking your pipeline.</p>
                ...
                <div key={stage.id} className="flex-shrink-0 w-64">
                  ...
                  <div className={cn(
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs bg-[var(--color-bg-secondary)] text-[var(--color-text-muted)] rounded-full px-1.5 py-0.5">
                        {stageDeals.length}
                      </span>
                    </div>
                    {stageValue > 0 && (
                      <span className="text-xs font-medium text-[var(--color-text-muted)]">{fmt$(stageValue, currency)}</span>
                    )}
                  </div>
                  ...
                  <button
                    ...
                    className="mt-2 w-full py-2 text-xs text-[var(--color-text-muted)] hover:text-[var(--color-accent)] hover:bg-[var(--color-bg-hover)] rounded-lg flex items-center justify-center gap-1 transition-colors"
                  >
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-[var(--color-border)]">
                {deals.map(deal => {
                  ...
                  const overdue = days !== null && days < 0 && deal.status === "open";
                  return (
                    <tr key={deal.id} className="hover:bg-[var(--color-bg-hover)] group">
                      <td className="px-4 py-3">
                        <Link href={`/deals/${deal.id}`} className="font-medium text-[var(--color-text-primary)] hover:text-[var(--color-accent)]">
                          {deal.title}
                        </Link>
                        {deal.status !== "open" && (
                          <span className={cn(
                      ...
                      <td className="px-4 py-3 text-[var(--color-text-muted)]">{deal.client_name ?? "—"}</td>
                      <td className="px-4 py-3">
                        <span className="flex items-center gap-1.5">
                          <span className="w-2 h-2 rounded-full" style={{ backgroundColor: deal.stage.color }} />
                          <span className="text-[var(--color-text-secondary)]">{deal.stage.name}</span>
                        </span>
                      </td>
                      <td className="px-4 py-3 font-medium text-[var(--color-text-primary)]">{fmt$(deal.value, deal.currency)}</td>
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2">
                      ...
                      <td className={cn("px-4 py-3 text-xs", overdue ? "text-red-600 font-medium" : "text-[var(--color-text-muted)]")}>
                        {deal.expected_close
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                          ...
                          <button onClick={() => setDelTarget(deal)}
                            className="p-1 rounded hover:bg-red-50 text-[var(--color-text-muted)] hover:text-red-600">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="bg-[var(--color-bg)] rounded-xl shadow-2xl w-full max-w-sm border border-[var(--color-border)] p-6">
            ...
            <div className="flex gap-3">
              <button onClick={() => setDelTarget(null)} className="flex-1 py-2 rounded-lg border border-[var(--color-border)] text-sm text-[var(--color-text-secondary)] hover:bg-[var(--color-bg-hover)]">Cancel</button>
